...
class html2text
{
    ...
    public $search = array(
            ...
            "/[\n\t]+/", // Newlines and tabs
            '/<head[^>]*>.*?<\/head>/i', // <head>
            '/<script[^>]*>.*?<\/script>/i', // <script>s -- which strip_tags
                                             // supposedly has problems with
            '/<style[^>]*>.*?<\/style>/i', // <style>s -- which strip_tags
                                           // supposedly has problems with
            '/<p[^>]*>/i', // <P>
            '/<br[^>]*>/i', // <br>
            '/<i[^>]*>(.*?)<\/i>/i', // <i>
    ...
    /**
     * Workhorse function that does actual conversion (calls _converter()
     * method).
     *
     * @access private
     * @return void
     ...
     *
     * @param
     *            string Reference to HTML content string
     *            
     * @access private
     * @return void
     */
    private function _converter (&$text)
    {
        ...
        // Remove unknown/unhandled entities (this cannot be done in
        // search-and-replace block)
        $text = preg_replace('/&([a-zA-Z0-9]{2,6}|#[0-9]{2,4});/', '', $text);
        
        // Convert "|+|amp|+|" into "&", need to be done after handling of
        // unknown entities
        ...
        // remove leading empty lines (can be produced by eg. P tag on the
        // beginning)
        $text = ltrim($text, "\n");
    ...
    private function _convert_pre (&$text)
    {
        ...
        while (preg_match('/<pre[^>]*>(.*)<\/pre>/ismU', $text, $matches)) {
            ...
            $this->pre_content = sprintf('<div><br>%s<br></div>', 
                    preg_replace($this->pre_search, $this->pre_replace, 
                            $this->pre_content));
            // replace the content (use callback because content can contain $0
            // variable)
    ...
    {
        if (preg_match_all('/<\/*blockquote[^>]*>/i', $text, $matches, 
                PREG_OFFSET_CAPTURE)) {
            $level = 0;
            $diff = 0;
            $start = 0;
                ...
                if ($m[0][0] == '<' && $m[0][1] == '/') {
                    ...
                    if ($level < 0) {
                        ...
                        if ($level > 0) {
                            ...
                            $body = substr($text, $start + $taglen - $diff, 
                                    $len);
                            ...
                            // Replace content
                            $text = substr($text, 0, $start - $diff) . $body .
                                     substr($text, $end + strlen($m[0]) - $diff);
                            
                            $diff = $len + $taglen + strlen($m[0]) -
                                     strlen($body);
                            unset($body);
                        }
                } else {
    ...
    private function _preg_callback ($matches)
    {
        switch (strtolower($matches[1])) {
            ...
            case 'a':
                ...
                $link_override = null;
                if (preg_match("/_html2text_link_(\w+)/", $matches[4], 
                        $link_override_match)) {
                    $link_override = $link_override_match[1];
                }
                // Remove spaces in URL (#1487805)
                $url = str_replace(' ', '', $matches[3]);
                return $this->_build_link_list($url, $matches[5], 
                        $link_override);
        }
    }
