 ...
 * ~ class.phpmailer.php
 * .---------------------------------------------------------------------------.
 * | Software: PHPMailer - PHP email class | | Version: 5.2.6 | | Site:
 * https://github.com/PHPMailer/PHPMailer/ | |
 * ------------------------------------------------------------------------- | |
 * Admins: Marcus Bointon | | Admins: Jim Jagielski | | Authors: Andy Prevost
 * (codeworxtech) codeworxtech@users.sourceforge.net | | : Marcus Bointon
 * (coolbru) phpmailer@synchromedia.co.uk | | : Jim Jagielski (jimjag)
 * jimjag@gmail.com | | Founder: Brent R. Matzelle (original founder) | |
 * Copyright (c) 2010-2012, Jim Jagielski. All Rights Reserved. | | Copyright
 * (c) 2004-2009, Andy Prevost. All Rights Reserved. | | Copyright (c)
...
class PHPMailer
{
    ...
    /**
     * Used with DKIM Signing
     * required if DKIM is enabled, in format of email address
     * 'you@yourdomain.com' typically used as the source of the email
     * 
     * @var string
     */
     ...
     * 
     * @var string NULL for default, whitespace for None, or actual string to
     *      use
     */
    public $XMailer = '';
    ...
    public static function ValidateAddress ($address)
    {
        if (defined('PCRE_VERSION')) { // Check this instead of extension_loaded
                                       // so it works when that function is
                                       // disabled
    ...
    public function PreSend ()
    {
        try {
            ...
            if (! empty($this->DKIM_domain) && ! empty($this->DKIM_private) &&
                     ! empty($this->DKIM_selector) && ! empty(
                            $this->DKIM_domain) &&
    ...
    protected function MailSend ($header, $body)
    {
        ...
        if ($this->SingleTo === true && count($toArr) > 1) {
            ...
            $rt = $this->mail_passthru($to, $this->Subject, $body, $header, 
                    $params);
            // implement call back function if it exists
            $isSent = ($rt == 1) ? 1 : 0;
    ...
    protected function SmtpSend ($header, $body)
    {
        ...
        foreach ($this->to as $to) {
            if (! $this->smtp->Recipient($to[0])) {
                ...
                // implement call back function if it exists
                $isSent = 0;
                $this->doCallback($isSent, $to[0], '', '', $this->Subject, 
                        $body);
            } else {
                // implement call back function if it exists
                $isSent = 1;
        ...
        foreach ($this->cc as $cc) {
            if (! $this->smtp->Recipient($cc[0])) {
                ...
                // implement call back function if it exists
                $isSent = 0;
                $this->doCallback($isSent, '', $cc[0], '', $this->Subject, 
                        $body);
            } else {
                // implement call back function if it exists
                $isSent = 1;
                ...
                try {
                    ...
                    if ($this->SMTPAuth) {
                        if (! $this->smtp->Authenticate($this->Username, 
                                $this->Password, $this->AuthType, $this->Realm, 
                                $this->Workstation)) {
                            throw new phpmailerException(
        ...
        while (! $foundSplitPos) {
            ...
            if ($encodedCharPos !== false) {
                ...
                if ($dec < 128) { // Single byte character.
                                  ...
                                  // encoded char
                    $maxLength = ($encodedCharPos == 0) ? $maxLength : $maxLength -
                             ($lookBack - $encodedCharPos);
                    $foundSplitPos = true;
                } elseif ($dec >= 192) { // First byte of a multi byte character
                                         // Reduce maxLength to split at start
                                         // of character
    ...
    protected function AttachAll ($disposition_type, $boundary)
    {
        ...
        foreach ($this->attachment as $attachment) {
            ...
            if ($attachment[6] == $disposition_type) {
                ...
                // quoted, but not otherwise: RFC2183 & RFC2045 5.1
    ...
    public function EncodeHeader ($str, $position = 'text')
    {
        ...
        if ($x > strlen($str) / 3) { // More than a third of the content will need
                                   // encoding, so B encoding will be most
                                   // efficient
            $encoding = 'B';
    ...
    public function EncodeQ ($str, $position = 'text')
    {
        ...
        switch (strtolower($position)) {
            ...
            default:
                ...
                // We put \075 (=) as first value to make sure it's the first
                // one in being converted, preventing double encode
                $pattern = '\075\000-\011\013\014\016-\037\077\137\177-\377' .
                         $pattern;
                break;
        }
    ...
    /**
     * Creates a message from an HTML string, making modifications for inline
     ...
     * @param bool $advanced
     *            Should this use the more complex html2text converter or just a
     *            simple one?
     * @return string
     */
    ...
    public static function _mime_types ($ext = '')
    {
        ...
        return (! isset($mimes[strtolower($ext)])) ? 'application/octet-stream' : $mimes[strtolower(
                $ext)];
    }

    /**
     * Try to map a file name to a MIME type, default to
    ...
    /**
     * Normalize UNIX LF, Mac CR and Windows CRLF line breaks into a single line
     * break format
