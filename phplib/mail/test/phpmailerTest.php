...
class phpmailerTest extends PHPUnit_Framework_TestCase
{
     ...
     * Needs a connection to a server that supports this auth mechanism, so
    ...
    function test_Quoted_Printable ()
    {
        ...
        // Check that a quoted printable encode and decode results in the same
        // as went in
        ...
        $this->assertEquals($this->Mail->EncodeQP($t), 
...
EOT;
    ...
    function test_MsgHTML ()
    {
        ...
        $this->assertNotEmpty($this->Mail->AltBody, 
                'AltBody not set by MsgHTML');
        $this->assertTrue($this->Mail->Send(), $this->Mail->ErrorInfo);
        ...
        $this->assertNotEmpty($this->Mail->AltBody, 
                'Advanced AltBody not set by MsgHTML');
    ...
    function test_Embedded_Image ()
    {
        ...
        if (! $this->Mail->AddEmbeddedImage('../examples/images/phpmailer.png', 
                'my-attach', 'phpmailer.png', 'base64', 'image/png')) {
            $this->assertTrue(false, $this->Mail->ErrorInfo);
            return;
        }
        ...
        $this->assertTrue($this->Mail->Send(), $this->Mail->ErrorInfo);
        // For code coverage
        $this->Mail->AddEmbeddedImage('thisfiledoesntexist', 'xyz'); // Non-existent
                                                                     // file
    ...
    {
        $this->assertTrue($this->Mail->SmtpConnect(), 
                'SMTP single connect failed');
        $this->Mail->SmtpClose();
        $this->Mail->Host = "localhost:12345;10.10.10.10:54321";
        $this->assertFalse($this->Mail->SmtpConnect(), 
                'SMTP bad multi-connect succeeded');
        $this->Mail->SmtpClose();
        $this->Mail->Host = "localhost:12345;10.10.10.10:54321;" .
                 $_REQUEST['mail_host'];
        $this->assertTrue($this->Mail->SmtpConnect(), 
                'SMTP multi-connect failed');
        $this->Mail->SmtpClose();
        $this->Mail->Host = $_REQUEST['mail_host'];
        // Need to pick a harmless option so as not cause problems of its own!
        // socket:bind doesn't work with Travis-CI
        $this->assertTrue(
     ...
     * According to the ticket, this should get stuck in a loop, though I can't
     * make it happen.
     */
    function test_DenialOfServiceAttack2 ()
    {
        ...
        $t = $this->Mail->WrapText(
                str_repeat('This should no longer cause a denial of service. ', 
                        30), 0);
    }
    ...
    function test_Addressing ()
    {
        $this->assertFalse($this->Mail->AddAddress(''), 
                'Empty address accepted');
        $this->assertFalse($this->Mail->AddAddress('', 'Nobody'), 
                'Empty address with name accepted');
        $this->assertFalse($this->Mail->AddAddress('a@example..com'), 
                'Invalid address accepted');
        $this->assertTrue($this->Mail->AddAddress('a@example.com'), 
                'Addressing failed');
        $this->assertFalse($this->Mail->AddAddress('a@example.com'), 
                'Duplicate addressing failed');
        $this->assertTrue($this->Mail->AddCC('b@example.com'), 
                'CC addressing failed');
        $this->assertFalse($this->Mail->AddCC('b@example.com'), 
                'CC duplicate addressing failed');
        $this->assertFalse($this->Mail->AddCC('a@example.com'), 
                'CC duplicate addressing failed (2)');
        $this->assertTrue($this->Mail->AddBCC('c@example.com'), 
                'BCC addressing failed');
        $this->assertFalse($this->Mail->AddBCC('c@example.com'), 
                'BCC duplicate addressing failed');
        $this->assertFalse($this->Mail->AddBCC('a@example.com'), 
                'BCC duplicate addressing failed (2)');
        $this->assertTrue($this->Mail->AddReplyTo('a@example.com'), 
                'Replyto Addressing failed');
        $this->assertFalse($this->Mail->AddReplyTo('a@example..com'), 
    ...
    function test_AddressEscaping ()
    {
        ...
        $this->assertTrue(
                (strpos($b, 
                        'To: "Tim \"The Book\" O\'Reilly" <foo@example.com>') !==
                         false));
    }
    ...
    function test_BCCAddressing ()
    {
        ...
        $this->BuildBody();
        $this->Mail->ClearAllRecipients();
        $this->assertTrue($this->Mail->AddBCC('a@example.com'), 
                'BCC addressing failed');
        $this->assertTrue($this->Mail->Send(), 'Send failed');
    }
    ...
    {
        $this->Mail->CharSet = 'iso-8859-1';
        $this->assertEquals('=A1Hola!_Se=F1or!', 
        ...
        $this->Mail->CharSet = 'UTF-8';
        $this->assertEquals('=C2=A1Hola!_Se=C3=B1or!', 
    ...
    function test_LineBreaks ()
    {
        ...
        $mixedsrc = "Hello\nWorld\rAgain\r\n";
        $target = "Hello\r\nWorld\r\nAgain\r\n";
        $this->assertEquals($target, PHPMailer::NormalizeBreaks($unixsrc), 
                'UNIX break reformatting failed');
        $this->assertEquals($target, PHPMailer::NormalizeBreaks($macsrc), 
                'Mac break reformatting failed');
        $this->assertEquals($target, PHPMailer::NormalizeBreaks($windowssrc), 
                'Windows break reformatting failed');
        $this->assertEquals($target, PHPMailer::NormalizeBreaks($mixedsrc), 
    ...
    {
        $this->assertEquals('application/pdf', PHPMailer::_mime_types('pdf'), 
        ...
        $this->assertFalse($this->Mail->set('x', 'y'), 
                'Invalid property set succeeded');
        $this->assertTrue($this->Mail->set('Timeout', 11), 
                'Valid property set failed');
        // Test pathinfo
        $a = '/mnt/files/飛兒樂 團光茫.mp3';
        $q = PHPMailer::mb_pathinfo($a);
        $this->assertEquals($q['dirname'], '/mnt/files', 
                'UNIX dirname not matched');
        $this->assertEquals($q['basename'], '飛兒樂 團光茫.mp3', 
                'UNIX basename not matched');
        $this->assertEquals($q['extension'], 'mp3', 
                'UNIX extension not matched');
        $this->assertEquals($q['filename'], '飛兒樂 團光茫', 
                'UNIX filename not matched');
        $this->assertEquals(PHPMailer::mb_pathinfo($a, PATHINFO_DIRNAME), 
                '/mnt/files', 'Dirname path element not matched');
        $this->assertEquals(PHPMailer::mb_pathinfo($a, 'filename'), 
                '飛兒樂 團光茫', 'Filename path element not matched');
        $a = 'c:\mnt\files\飛兒樂 團光茫.mp3';
        $q = PHPMailer::mb_pathinfo($a);
        $this->assertEquals($q['dirname'], 'c:\mnt\files', 
                'Windows dirname not matched');
        $this->assertEquals($q['basename'], '飛兒樂 團光茫.mp3', 
                'Windows basename not matched');
        $this->assertEquals($q['extension'], 'mp3', 
                'Windows extension not matched');
        $this->assertEquals($q['filename'], '飛兒樂 團光茫', 
                'Windows filename not matched');
    }
}
