 ...
 *
 * Implements all the RFC 821 SMTP commands except TURN which will always return
...
class SMTP
{
     ...
     * @param int $port
     *            The port number to connect to, or use the default port if not
     *            specified
     * @param int $timeout
    ...
    public function Connect ($host, $port = 0, $timeout = 30, $options = array())
    {
        ...
        // Need to suppress errors here as connection failures can be handled at
        // a higher level
            ...
            $workstation = '')
    {
        ...
        require_once 'extras/ntlm_sasl_client.php';
                ...
                if (! $ntlm_client->Initialize($temp)) { // let's test if every
                                                       // function its available
                    $this->error = array(
                            'error' => $temp->error
                    );
                    if ($this->do_debug >= 1) {
                ...
                $challenge = substr($rply, 3); // though 0 based, there is a
                                               // white space after the 3 digit
                                               // number....//msg2
    ...
    /**
     * Works like hash_hmac('md5', $data, $key) in case that function is not
     * available
     * 
     * @access protected
     * @param string $data            
